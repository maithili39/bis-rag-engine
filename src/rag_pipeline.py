...
from config import TOP_K_RESULTS, CHROMA_PERSIST_DIR, DATASET_PDF
...
class BISRAGPipeline:
    ...
    def process_query(self, query: str, top_k: int = TOP_K_RESULTS) -> Tuple[List[str], str, float]:
        ...
        retrieved_docs, retrieval_latency = self.retriever.retrieve(query, k=15)
    ...
    def process_query_detailed(self, query: str, top_k: int = TOP_K_RESULTS) -> Dict[str, Any]:
        ...
        results_with_scores, retrieval_latency = self.retriever.retrieve_with_scores(query, k=15)
